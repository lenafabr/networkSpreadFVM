...
This example calculates calcium release assuming rapid binding equilibration with buffer proteins. The code evolves two spatiotemporal fields: (1) the free calcium concentration and (2) the buffer binding site concentration, which in this simple case is uniform throughout
...
\begin{itemize}
	...
	\item Applies when using STARTNODES or STARTPOS
\end{itemize}
%
\item {\it CEXT}
\begin{itemize}
...
\item {\it FASTEQUIL}
\begin{itemize}
	\item 1 logical. Default F
	\item Assume fast equilibration of binding of ions to buffer protein sites
	\item If this is True, then there will be 2 fields: (1) free ligands, (2) total buffer sites
\end{itemize}
%
\item {\it FIXNODE}
\begin{itemize}
...
\end{itemize}
%
\item {\it PERIODICGLOBALPERM}
\begin{itemize}
	\item  value: 3 or more floats; default: turned off
	\item Set up periodic global release from the network. Every so often, all mesh cells become permeable and remain so for a certain duration of time, then stop being permeable again.
	\item Values are:
	\begin{itemize}
		\item Period for becoming permeable (units of time)
		\item Duration of permeable interval (units of time)
		\item Permeability during the permeable interval for each of the fields. Provide one value for each field. If PERMPREFACTOR is turned on this is a permeability prefactor ($2P/(aD)$ if working with 1D concentrations). See PERMNODE keyword documentation for details on how permeability values are defined.
	\end{itemize}	
\end{itemize}
%
...
\item {\it PERMPOS}
\begin{itemize}
	\item  value: 3-4 floats; default: 0 0 0
	\item Center permeable region around a particular set of coordinates
	\item First 2-3 values are the coordinates (should be same number as the network dimensionality). Last value is the permeability of the region centered on these coordinates.
	\item Radius of permeable region is set by PERMNEARNODEDIST (see caveats for that keyword about graph distance vs spatial distance)
\end{itemize}
...
\item {\it RANDFIXRESV}
\begin{itemize}
	\item 2 integers, 1 floats; default: none
	\item Fix a particular field on a random subset of reservoirs (selected without replacement)
	\item Values are: 
	\begin{enumerate}
		\item Which field to fix?
		\item How many reservoirs to fix?
		\item What concentration to fix those reservoirs to?
	\end{enumerate}
\end{itemize}

\item {\it RESVINFO}
\begin{itemize}
	\item 1 integer, 3 floats; default: none
	\item Information about a well-mixed reservoir
	\item Integer: which reservoir?
	\item Parameters:
	\begin{enumerate}
		\item Volume, rescaled for 1D calculations as $V_{1D} = V/(\pi a^2)$ where $a$ is tubule radius. If CONCENTRATIONS3D is set, this should just be the 3D volume.
		\item Surface area, rescaled for 1D calculations as $A_{1D} = A/(2\pi a^2)$. If CONCENTRATIONS3D is set, this should just be the normal surface area.
		\item Effective length for defining flux between adjacent mesh elements. For a flat sheet this is $(a^2 /h) \log \hat{R}/a$, where $\hat{R}$ is the radius of the flat sheet, or is $\hat{R} = \sqrt{A_\text{nuc}/\pi}$ if the sheet is wrapped around a spherical nucleus. For a sphere, the effective length is $\pi a/2$, regardless of the sphere size.
	\end{enumerate}
...
\begin{itemize}
	...
	\item If running equilibrated binding (FASTEQUIL), then the 2 concentrations are: (1) free ion, (2) total buffer sites
