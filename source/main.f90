...
CONTAINS  
  ...
  SUBROUTINE RUNDYNAMICSDRIVER(NETP,ACTION)
    ...
    USE KEYS, ONLY : NSTEP, DELT, PRINTEVERY, SNAPSHOTEVERY, SNAPSHOTFILE, &
         ...
         & EDGERADRANDTYPE, EDGERADRANDPARAMS, &
         & USERESVELEMENTS, RESVELEMENTFILE, CONCENTRATIONS3D, &
         & USEGLOBALRESV,GLOBALRESVOL,GLOBALRESVSTART, &
         & EDGERADVARTYPE,EDGERADVARPARAMS, EDGERADRANDTYPE
    ...
    IF (EDGERADRANDTYPE.NE.'NONE') THEN
    ...
    IF (USERESVELEMENTS) THEN
       CALL SETUPNETWORKMESH(MESHP,NETP,MAXMESHSIZE,MINMESHPT,CONC3D,&
            & USEGLOBALRESV,GLOBALRESVOL,EDGERADVARTYPE,EDGERADVARPARAMS,EDGERADRANDTYPE,RESVP)
    ELSE
       CALL SETUPNETWORKMESH(MESHP,NETP,MAXMESHSIZE,MINMESHPT,CONC3D,&
            & USEGLOBALRESV,GLOBALRESVOL,EDGERADVARTYPE,EDGERADVARPARAMS,EDGERADRANDTYPE)
    ENDIF        
    ...
    ELSEIF(NSTARTPOS.GT.0) THEN ! Initialize near specific positions
       CALL INITIALIZEFIELDNEARPOS(NETP,DSP,STARTPOS(1:NSTARTPOS,1:NETP%DIM),INITCONC,STARTNODERAD,BACKGROUNDCONC(1:DSP%NFIELD))
    ...
    IF (DSP%MESHP%USEGLOBALRESV) THEN
       !PRINT*, 'TESTX1:', MESHP%GLOBALRESVIND, MESHP%NCELL, SHAPE(DSP%FIELDS)
       CC = DSP%MESHP%GLOBALRESVIND
       DSP%FIELDS(CC,:) = 0D0
       DSP%FIELDS(CC,1) = GLOBALRESVSTART
    ENDIF
