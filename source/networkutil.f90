MODULE NETWORKUTIL
  ...
  TYPE NETWORK
     ...
     DOUBLE PRECISION, POINTER :: RESVVOL(:) ! volume of each reservoir
...
CONTAINS
  ...
  SUBROUTINE JUMPNODE(NETP,NODE,TARGETEDGE,EDGELOC)
    ! move a degree two node from one edge to another
    ! NODE: node index which to move
    ! TARGETEDGE: target edge to which to move node
    ! EDGELOC: fractional location along edge
    ! The original node should have edge 1 pointing in and edge 2 pointing out
    ! does *not* update contraction structures
    USE GENUTIL, ONLY : NORMALIZE
    
    IMPLICIT NONE
    TYPE(NETWORK), POINTER :: NETP
    INTEGER, INTENT(IN) :: NODE, TARGETEDGE
    DOUBLE PRECISION, INTENT(IN) :: EDGELOC
    INTEGER :: EDGE1, EDGE2, NSTART, NEND, N1,N2, I
    DOUBLE PRECISION :: TARGETLEN, TOTLEN

    IF (NETP%NODEDEG(NODE).NE.2) THEN
       PRINT*, 'ERROR IN JUMPNODE: not a degree 2 node', NETP%NODEDEG(NODE)
       STOP 1
    ENDIF

   ! PRINT*, 'TESTX4:', NODE, TARGETEDGE, EDGELOC
   ! PRINT*, 'TESTX5:', NETP%EDGEDIR(TARGETEDGE,:)

    ! get the 2 edges currently attached to this node
    EDGE1 = NETP%NODEEDGE(NODE,1)
    EDGE2 = NETP%NODEEDGE(NODE,2)
    
    ! end points of old edge
    NSTART = NETP%EDGENODE(EDGE1,1)
    NEND = NETP%EDGENODE(EDGE2,2)
     IF (NSTART.EQ.NODE.OR.NEND.EQ.NODE) THEN
        PRINT*, 'ERROR IN JUMPNODE: moving node should have first edge pointing in, second pointing out'
        PRINT*, 'NODE EDGE1 EDGE2:', NODE, EDGE1, EDGE2
        PRINT*, 'NSTART, NEND:', NSTART, NEND
       STOP 1
    ENDIF

    ! start and end point of new edge
    N1 = NETP%EDGENODE(TARGETEDGE,1)
    N2 = NETP%EDGENODE(TARGETEDGE,2)
    TARGETLEN = NETP%EDGELEN(TARGETEDGE)
    
    TOTLEN = NETP%EDGELEN(EDGE1)+NETP%EDGELEN(EDGE2)

    ! update node position  
    NETP%NODEPOS(NODE,:) = NETP%NODEPOS(N1,:) + NETP%EDGEDIR(TARGETEDGE,:)*TARGETLEN*EDGELOC
    
    
    ! change node position only, do not need to reconnect
    IF (EDGE1.EQ.TARGETEDGE) THEN      
       NETP%EDGELEN(EDGE1) = TARGETLEN*EDGELOC
       NETP%EDGELEN(EDGE2) = TOTLEN - TARGETLEN*EDGELOC
       RETURN
    ELSEIF (EDGE2.EQ.TARGETEDGE) THEN       
       NETP%EDGELEN(EDGE1) = NETP%EDGELEN(EDGE1) + NETP%EDGELEN(EDGE2)*EDGELOC
       NETP%EDGELEN(EDGE2) = NETP%EDGELEN(EDGE2) * (1-EDGELOC)
       RETURN
    ENDIF
    ...
    ! reconnect original edge (using first edge index)
    NETP%EDGENODE(EDGE1,2) = NEND
    NETP%EDGELEN(EDGE1) = TOTLEN
    DO I = 1,NETP%NODEDEG(NEND)
       IF (NETP%NODEEDGE(NEND,I).EQ.EDGE2) THEN
          NETP%NODEEDGE(NEND,I) = EDGE1
          NETP%NODENODE(NEND,I) = NSTART
          EXIT
       ENDIF
    ENDDO
    DO I= 1,NETP%NODEDEG(NSTART)
       IF (NETP%NODEEDGE(NSTART,I).EQ.EDGE1) THEN
          NETP%NODENODE(NSTART,I) = NEND
       ENDIF
    ENDDO
    ! reset edge direction
    NETP%EDGEDIR(EDGE1,:) = NETP%NODEPOS(NEND,:)-NETP%NODEPOS(NETP%EDGENODE(EDGE1,1),:)
    CALL NORMALIZE(NETP%EDGEDIR(EDGE1,:))

    ! update any loops containing the now-merged edge
    CALL UPDATELOOP_EDGEMERGE(NETP,EDGE1)    
   
    ! split up the target edge into 2
    ! first part keeps its original edge index
    NETP%EDGENODE(TARGETEDGE,2) = NODE
    NETP%EDGENODE(TARGETEDGE,1) = N1
    
    NETP%EDGELEN(TARGETEDGE) = TARGETLEN*EDGELOC
    DO I = 1,NETP%NODEDEG(N1)
       IF (NETP%NODEEDGE(N1,I).EQ.TARGETEDGE)  NETP%NODENODE(N1,I) = NODE
    ENDDO
    ! reset edge direction
    NETP%EDGEDIR(TARGETEDGE,:) = NETP%NODEPOS(NODE,:)-NETP%NODEPOS(N1,:)
    CALL NORMALIZE(NETP%EDGEDIR(TARGETEDGE,:))
    
    ! second part gets the edge2 index
    NETP%EDGENODE(EDGE2,1) = NODE
    NETP%EDGENODE(EDGE2,2) = N2
    NETP%EDGELEN(EDGE2) = TARGETLEN*(1-EDGELOC)
    DO I = 1,NETP%NODEDEG(N2)
       IF (NETP%NODEEDGE(N2,I).EQ.TARGETEDGE) THEN
          NETP%NODEEDGE(N2,I) = EDGE2
          NETP%NODENODE(N2,I) = NODE
       ENDIF
    ENDDO
    ! reset edge direction
    NETP%EDGEDIR(EDGE2,:) = NETP%NODEPOS(N2,:)-NETP%NODEPOS(NODE,:)
    CALL NORMALIZE(NETP%EDGEDIR(EDGE2,:))
    
    NETP%NODENODE(NODE,:) = (/N1,N2/)
    NETP%NODEEDGE(NODE,:) = (/TARGETEDGE,EDGE2/)

    ! Update any loops containing the newly split edge
    CALL UPDATELOOP_EDGESPLIT(NETP,TARGETEDGE,EDGE2)    
    
  END SUBROUTINE JUMPNODE
  ...
  SUBROUTINE NETWORKFROMFILE(NETP,NETFILE)
    ...
    USE KEYS, ONLY : MAXBRANCH,  NABS, ABSORBERS, &
         ...
         & NODEVOL,RESVVOL, FIXEDGEVEL,FIXEDGEVELVAL,NFIXEDGEVEL, FIXNODES, FIXVALS, NFIX, &
...
!    WRITE(FU,'(A)') '#list of node indices and positions'
  ...
  SUBROUTINE SETUPNETWORK(NETP,NNODE,NEDGE,NRESV,NLOOP,DIM,NFIELD,&
    ...
    ALLOCATE(NETP%NODERESV(NNODE),NETP%RESVNODES(NRESV,NNODE),&
         & NETP%RESVNNODE(NRESV),NETP%RESVVOL(NRESV),NETP%RESVMIX(NRESV))
    NETP%NODERESV = 0
    IF (NRESV.GT.0) THEN      
       NETP%RESVNNODE = 0
       NETP%RESVMIX = .TRUE.
...
!       DEALLOCATE(NETP%NODEFIX, NETP%NODEFIXVAL)
       ...
       DEALLOCATE(NETP%NODERESV,NETP%RESVNODES,NETP%RESVVOL, &
            & NETP%RESVMIX, NETP%ISPERM, NETP%RESVCELLS)
