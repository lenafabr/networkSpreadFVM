MODULE NETWORKUTIL
  ...
  TYPE NETWORK
     ...
     ! volume and surface area of each reservoir
...
CONTAINS
  ...
  SUBROUTINE NETWORKFROMFILE(NETP,NETFILE)
    ...
    USE KEYS, ONLY : MAXBRANCH,  NABS, ABSORBERS, &
         ...
         & NODEVOL,RESVVOL, RESVSA,FIXEDGEVEL,FIXEDGEVELVAL,NFIXEDGEVEL, FIXNODES, FIXVALS, NFIX, &
...
!    WRITE(FU,'(A)') '#list of node indices and positions'
  ...
  SUBROUTINE SETUPNETWORK(NETP,NNODE,NEDGE,NRESV,NLOOP,DIM,NFIELD,&
    ...
    ALLOCATE(NETP%NODERESV(NNODE),NETP%RESVNODES(NRESV,NNODE),&
         & NETP%RESVNNODE(NRESV),NETP%RESVVOL(NRESV),NETP%RESVSA(NRESV),NETP%RESVMIX(NRESV))
    NETP%NODERESV = 0
    IF (NRESV.GT.0) THEN      
       NETP%RESVNNODE = 0
       NETP%RESVMIX = .TRUE.
