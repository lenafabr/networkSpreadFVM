...
CONTAINS  
  ...
  SUBROUTINE PRINTMAT(MAT)
    ! print matrix in row-column form
    IMPLICIT NONE
  ...
  SUBROUTINE CUMSUM(LIST,NL,CUMSUMLIST)
    ! get the cumulative sum of a double precision list
    IMPLICIT NONE
    INTEGER, INTENT(IN) :: NL
    DOUBLE PRECISION, INTENT(IN) :: LIST(NL)
    DOUBLE PRECISION, INTENT(OUT) :: CUMSUMLIST(NL)
    INTEGER :: I
    ...
    CUMSUMLIST(1) = LIST(1)
    DO I = 2,NL
       CUMSUMLIST(I) = CUMSUMLIST(I-1)+LIST(I)
    ENDDO
  ...
  SUBROUTINE NORMALIZE(X,NORMX)
    ! normalize a vector

    IMPLICIT NONE

    DOUBLE PRECISION, INTENT(INOUT) :: X(:)
    DOUBLE PRECISION, INTENT(OUT), OPTIONAL :: NORMX
    DOUBLE PRECISION :: DX
  ...
  SUBROUTINE RANDSELECT_INT(LIST,NPICK,REPLACE,VALS,INDS)
    ...
    IF (.NOT.REPLACE.AND.NPICK.GT.SIZE(LIST)) THEN
       PRINT*, 'ERROR IN RANDSELECT_INT: cannot pick that many values without replacement', NPICK, SIZE(LIST)
       STOP 1
    ENDIF
   
    AVAILABLE = .TRUE.
    INDLIST = (/(I, I=1,SIZE(LIST))/)
